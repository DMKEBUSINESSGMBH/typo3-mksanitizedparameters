...
- common rules (__common): will be used if there is no special rule for a parameter

- default rules (__default): will be used if there is neither a special rule nor a common rule
...
Filter configs with a array which contains the key "filter" have higher priority than filter configs with a array which contains several filter as array. The highest priority have single filter configs. see :ref:`orderOfRules`
